//Activyty: You are planning a car trip so you post on a carpooling website in order to share the cost of the trip.
...
int main(void){
    ...
    //Lets scan from the user and asing the value to the variables.
    scanf("%lf %lf", &passangers,&cost);
    
    if(passangers<1){
        printf("%.2lf",cost);
    }else{
        cost = (1 + cost)/(passangers + 1); 
        printf("%.2lf",cost);
    }
